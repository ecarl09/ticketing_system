...
@section('current-page-title') User Validation @endsection
.@section('current-page-sub-title') List of unverified users! @endsection
@section('current-page-title-icon') fas fa-user-lock @endsection
                        ...
                        <tbody class="list" id="table-customers-body">
                            @foreach($users as $data)
                                <tr class="btn-reveal-trigger">
                                    ...
                                    <td class="dateCreated align-middle white-space-nowrap py-2">{{$data->isVerified === 'NO' ? 'UN VERIFIED' : NULL }}</td>

                                    <td class="align-middle white-space-nowrap py-2 text-end">
                                        <a class="btn btn-primary btn-sm" href="{{ route('verify.user', $data->id) }}">Verify</a>
                                    </td>
                                </tr>
                            @endforeach   
                        </tbody>
...
@push('javascript')
    ...
    <script>
        $(document).ready(function() {
            var t = $('#default-datatable').dataTable( {
                "order": [6, 'desc'],
            });
        });
    </script> -->
